...
@ %def 
Reported as bug at \url{https://bugs.r-project.org/bugzilla/show_bug.cgi?id=15481}.
...
\item Check why loading developer version with \code{load_all} these messages are issued:
\begin{verbatim}
