...
\item Check warnings returned when detaching the package:
    @ 
<<eval=F>>=
...
moths.rad.pred2 <- radpred(object=moths, sad="ls", 
                             coef=list(alpha=35), distr="D")
moths.rad.pred3 <- radpred(object=moths, sad="ls", 
                             coef=list(alpha=35), distr="D", S=length(moths), N=sum(moths))
plot(moths.rad.pred)
lines(moths.rad.pred2, col="black")
...
\item Conferir funcao trueLL e seu uso na fitsad, pois pelo menos para dados de mariposas de Fisher
  resulta em melhoria do modelo lognormal truncado, mas o grafico de oitavas nao mostra isto
  @ 
<<TrueLL>>=
## TRUE likelihood 
...
## o melhor desempenho do lognormal
plot(moths.oc)
lines(octavpred(moths.pln))
lines(octavpred(moths.ln1), col="red")
## E qqplots mostram melhor ajuste da Logserie
@ %def 
...
\item Check if parameters J for mszm and volkov and N for ls can be declared as fixed in their fitting functions. This will also allow to eliminate a bunch of \code{if}'s in \code{radpred} and \code{octavpred}. 
\end{itemize}
...
\item Include Preston's method for distribution of boundary values among octaves in \code{octav} and \code{octavpred}

\item Rewrite \code{octav} and \code{octavpred} to use \code{hist} instead of \code{cut}, 
  see example of dzsm and help page of cut: ``Instead of ‘table(cut(x, br))’, ‘hist(x, br, plot = FALSE)’ is
  more efficient and less memory hungry.''. Also, more handy to choose midpoints and different classing methods (e.g. rigth=T or FALSE in cut)
\end{itemize}
