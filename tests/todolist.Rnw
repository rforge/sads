...
\item Quantile functions of rad models (\code{qbss}, \code{qgs}, \code{qmand} and \code{qzipf})  extremely slow.
  ...
  \end{itemize}
\item ensure that all mehods for data frames work with classes \code{octav} and \code{rad}.
...
\item Summary of \code{fivolkov} with extremelly long call,
  probably due the use of do.call in the function:
@ 
<<summary-volkov>>=
...
\begin{itemize}
  ...
  \item Poisson-gamma (mayne, since it is negative binomial)
...
\item \textbf{DONE} Include Preston's method for distribution of boundary values among octaves in \code{octav} and \code{octavpred}
...
\item Rewrite \code{octav} and \code{octavpred} to use \code{hist} instead of \code{cut}, 
  ...
  Also, more handy to choose midpoints and different classing methods (e.g. rigth=T or FALSE in cut)
