...
#(moths.nb <- fitsad(moths,"nbinom"))## nao entendi
(moths.gm <- fitsad(moths,"gamma"))
@ %def 

@ 
<<Model selection table>>=
AICtab(moths.ls, moths.pl, moths.nb, moths.gm)
@ %def 

@ 
<<Graphical comparison: octaves>>=
head(moths.ls.oc <- octavpredt(moths.ls)) # verificar se permanece octavpredt
head(moths.pl.oc <- octavpredt(moths.pl))
plot(moths.oc)
lines(moths.ls.oc, col="blue")
lines(moths.pl.oc, col="red")
legend("topright", c("Logseries", "Poisson-lognormal"), lty=1, 
       col=c("blue","red"))
@ %def 
...
@ 
<<Graphical comparison: octaves>>=
head(moths.ls.rad <- radpredt(moths.ls)) # verificar se permanece radpredt
