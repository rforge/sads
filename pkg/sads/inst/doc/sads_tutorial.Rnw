...
The first guess on investigating this integration was to verify the details of the integrate function. 
From the associated help we found that it implements the adaptative quadrature method. Beyond the 
expression to be integrated and limits, integrate() also allow to specify the absolute and relative 
accuracy of the function used to approximate the integrand 
(http://www.amtp.cam.ac.uk/lab/people/sd/lectures/nummeth98/integration.htm).

After some trials we found that substitutig 'Inf' by particular values let the numerical solution matches 
the anlytical. It is observated that increasing the upper limit when FRAC and RATE are getting lower 
conduct to better aproximation of the numerical integration. Finally, considering the range of values 
10e-12<FRAC<1 and 10e-6<RATE<1, the proposed solution is to replace Inf by 100/(a+lambda).
