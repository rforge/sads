...
With a few trials we found that substitutig \code{Inf} by particular values, this can make the numerical solution matches the anlytical. It is observated that increasing the upper limit, when \code{frac} and \code{rate} are getting lower,  conduct to better aproximation of the numerical integration. Considering $\code{frac}=10^{-1}$ and $\code{rate}=10^{-5}$, with $200 \leq \code{y} \geq 20000$, a reasonably good choice is replace \code{Inf} by \code{k*y/(a+lambda)} with \code{k=1.9920941}. Figure \ref{fig:fig1} shows the analytical and numerical curves visually matching. Figure \ref{fig:fig0} is the result when integral is calculated with \code{abs.tol}.
...
the upper limit of integration in an another alternative manner. This time the idea is to split the total range of any parameter, \code{y} for 
example, in two or more parts and then specify the numerical integration for each part. The updated function for numerical integration is below.
...
@
The above expression is an attempt to correct the result from the numerical  integration provided by the standart \code{integrate} R function. The limits for each \code{y} intervals were selected as example.

In fact, this is not a final solution yet. Both curves do not match for $\code{rate}=10^{-6}$\ and\ $\code{frac}=10^{-1}$, for example, as shows the graph of Figure \ref{fig:fig3} below:
...
Given that \code{frac} is a fraction of the total number of individuals of the comunity, that was included in the sample. This give us a upper limit on the horizontal axis and consequently an upper limit for the integration \code{y=1/frac}.

Also, independently of the limit of integration, the integral of the function for that interval must be 1. This aspects are verified:
